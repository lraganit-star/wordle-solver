...
(async () => {
    try {
        ...
        function processWord(count) {
            // instead of createColorArr, need to be able to use colorArr created in App.jsx
            var colorArr = []
            if (count < 6) {
...
// instead of using this function, should now make a new function 
// to import colorArr from App.jsx
...
function reduceWordList (bestWord, wordList, colorArr) {
    ...
    for (i in colorList) {
        ...
        if (colorList[i] =="green") {
            const filteredListGreen = justWords.filter(word => word.split("")[i] == letters[i])
        ...
        if (colorList[i] == "gray" || colorList[i] == "grey" ) {
            const filteredListGray = justWords.filter(word => !word.includes(letters[i]))
